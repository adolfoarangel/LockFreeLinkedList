...
CPPFLAGS += -Iinclude -I. -I$(SCFENCE_DIR) -std=c++11
...
include $(LockFreeLinkedList/Makefile

-include $(wildcard $(LockFreeLinkedList)/.*.d)
...
PHONY += clean
clean:
	rm -f *.o *.so .*.d *.pdf *.dot $(LockFreeLinkedList)/.*.d $(LockFreeLinkedList)/*.o
